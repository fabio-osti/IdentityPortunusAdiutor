...
public interface IUserManager<TUser, TRole, TKey>
...
where TKey : IEquatable<TKey>
{
	...
	TUser? CreateUser(Func<TUser, bool> userFinder, Func<TUser> userBuilder);
	/// <summary>
	...
	TUser? ValidateUser(Func<TUser, bool> userFinder, string userPassword);
	/// <summary>
	...
	TUser? SendEmailConfirmation(Func<TUser, bool> userFinder);
	/// <summary>
	...
	TUser? ConfirmEmail(Func<TUser, bool> userFinder);
	/// <summary>
	/// 	Sends a message to the <typeparamref name="TUser"/> found by
	...
	/// <param name="userFinder">Predicate for finding an <typeparamref name="TUser"/>.</param>
	/// <returns>The user if message sent, null else.</returns>
	TUser? SendPasswordRedefinition(Func<TUser, bool> userFinder);
	/// <summary>
	...
	TUser? RedefinePassword(Func<TUser, bool> userFinder, string newPassword);
}